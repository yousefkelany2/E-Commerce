...
<?php 
   ...
   $num_product=$products->num_rows;

    
    $page=$_GET["page"]??1;
    $start=($page-1)*4;
    $products=$connect->query("SELECT * FROM products LIMIT $start,4 ");
...
<div class="product-image">
    ...
    $images=explode('/',$product["image"]);
     echo " <img width=250 height=250  src='admin/images/$images[0]' alt='#'>"
    /* foreach($images as $image){
      echo " <img src='admin/images/$image' alt='#'>" ;
    } */
   
    ?>

<div class="button">
    <?php if(!isset($_SESSION["loginpro"])): ?>
   <a href="login.php" class="btn"><i class="lni lni-cart"></i> Add to Cart</a>
   <?php else :?>
   
   <bottom product_id=<?=$product["id"]?> class="btn  btncard"><i class="lni lni-cart"></i> Add to Cart</bottom>
 <?php endif?>
</div>
</div>
...
<h4 class="title">
<a href="product-grids.php"><?=$product["catogory"]?></a>
</h4>
<ul class="review">
...
<div class="number">
<?php
$numpro=$num_product/4;
for($i=1;$i<=ceil($numpro);$i++):
    $bar=$i; 
  ?>
  <a href="?page=<?=$i?>"><?=$i?></a>
